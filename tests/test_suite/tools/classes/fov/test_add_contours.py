...
def test_geo_contour_errors(mock_show, plot_cleanup, at):
    ...
    with pytest.raises(ValueError) as e_info:
        fov_map.add_geo_contours(lats=[60.0],
                                 lons=[-130.0],
                                 color="some_non_existent_color",
                                 linewidth=2,
                                 linestyle="--",
                                 marker="o",
                                 bring_to_front=True)
    ...
    with pytest.raises(ValueError) as e_info:
        fov_map.add_geo_contours(lats=[60.0],
                                 ...
                                 color="red",
                                 linewidth=2,
                                 linestyle="some_non_existent_linestyle",
                                 marker="o",
                                 bring_to_front=True)
    ...
    with pytest.raises(ValueError) as e_info:
        fov_map.add_geo_contours(lats=[60.0],
                                 ...
                                 linewidth=2,
                                 linestyle="--",
                                 marker="some_non_existent_marker",
                                 bring_to_front=True)
...
def test_mag_contour_errors(mock_show, plot_cleanup, at):
    ...
    # Attempt to add contours with incorrect color
    with pytest.raises(ValueError) as e_info:
        fov_map.add_mag_contours(timestamp,
                                 lats=[60.0],
                                 lons=[-130.0],
                                 color="some_non_existent_color",
                                 linewidth=2,
