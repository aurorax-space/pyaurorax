...
from ....search.api import URL_SUFFIX_EPHEMERIS_REQUEST
...
def __create_search_object_from_query(aurorax_obj, q):
    ...
    s = pyaurorax.search.EphemerisSearch(aurorax_obj,
                                         start,
                                         end,
                                         programs=programs,
                                         platforms=platforms,
...
@ephemeris_group.command("get_status", short_help="Get status info for an ephemeris search request")
...
@ephemeris_group.command("get_logs", short_help="Get logs for an ephemeris search request")
@click.argument("request_uuid", type=str)
...
def get_data(config, request_uuid, outfile, output_to_terminal, indent, minify):
    ...
    REQUEST_UUID    the request unique identifier
    """
    get_search_data(config.aurorax, "ephemeris", request_uuid, outfile, output_to_terminal, indent, minify)


@ephemeris_group.command("search_resubmit", short_help="Resubmit an ephemeris search request")
@click.argument("request_uuid", type=str)
@click.pass_obj
...
@ephemeris_group.command("search_template", short_help="Output template for an ephemeris search request")
@click.option("--outfile", type=str, help="Save template to a file")
@click.option("--indent", type=int, default=2, show_default=True, help="Indentation to use when outputing template")
@click.pass_obj
...
@ephemeris_group.command("search", short_help="Perform an ephemeris search request")
...
def search(config, infile, poll_interval, outfile, output_to_terminal, indent, minify, quiet):
    ...
    # start search
    s = config.aurorax.search.ephemeris.search(start,
                                               end,
                                               programs=programs,
                                               platforms=platforms,
                                               instrument_types=instrument_types,
    ...
    get_search_data(config.aurorax, "ephemeris", s.request_id, outfile, output_to_terminal, indent, minify, show_times=True, search_obj=s)


@ephemeris_group.command("describe", short_help="Describe an ephemeris search request")
@click.argument("infile", type=str)
@click.pass_obj
def describe(config, infile):
    ...
    # describe the search
    d = config.aurorax.search.ephemeris.describe(s)
