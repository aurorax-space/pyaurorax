...
def scale_intensity(
    ...
    Args:
        data (numpy.ndarray): data array, can be 2, 3, or 4-dimensional. Assumed 
            to be an image, or array of images. Also assumed that the first 2 dimensions
            are the image's x and y coordinates, and the following dimensions are some
            combination of the number of images, and/or the colour channel.
        min (float): minimum value of array to be considered
        max (float): maximum value of array to be considered
        top (float): maximum value of the scaled result. If not supplied, the max value
